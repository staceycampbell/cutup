...
static void
GetNames(WINDOW *old_w, char *you, char *hom)
{
	WINDOW *namesW;
	int you_len, hom_len;
	char test1[FN_MAX];
	...
	else
	{
		...
		if (SAME(test1, test2))
		{
			DISPLAY_ERROR(namesW, "Concatenated names are bad - try again.");
			GetNames(old_w, you, hom);
		}
	}
...
static void
ErrorExit(WINDOW *old_w, int status)
{
	EndFIFO();
	if (!StopIO)
		EndScreen();
...
static void
EndGame(WINDOW *old_w, char *you, char *hom)
{
	WINDOW *final_w;
	...
	if (!StopIO)
	{
		sprintf(mesg, "%s: %d - %s: %d ", you, OurScore, hom, TheirScore);
		len = strlen(mesg);
		max_len = len < sizeof hdr ? sizeof hdr : len;
		max_len += 3;
		final_w = CreateWindow(old_w, 5, max_len, 9, SM_CENTRE(max_len, COLS));
		wstandout(final_w);
...
static void
ProcessArgs(WINDOW *old_w, int argc, char **argv)
{
	int found;
	char mess[23];
...
static WINDOW *
InitBoard(WINDOW *play_w, int board[Y_MAX][X_MAX], coords_t *us, coords_t *them, coords_t *c1, coords_t *c2)
{
	WINDOW *board_w = 0;
	int i, j;
...
static void
DecisionScreen(WINDOW *old_w, WINDOW *board_w, int col_mask, char *you, char *hom)
{
	WINDOW *decisW;
	...
	wrefresh(old_w);
	if ((we_collide = col_mask & S_WE_COLLIDE) != 0)
		++TheirScore;
	if ((they_collide = col_mask & S_THEY_COLLIDE) != 0)
		++OurScore;
...
main(int argc, char **argv)
{
	...
	if ((status = GetFIFOName(play_w, &p_write, &p_read, you, hom, &c1, &c2)) != 0)
	...
	do
	{
		...
		do
		{
			collision = GetMoves(board_w, board, p_read, p_write, &us, &them);
			DrawPositions(board_w, board, &us, &them);
		}
		while (!collision);
		DecisionScreen(play_w, board_w, collision, you, hom);
	}
	while (((OurScore < SCORE_MAX && TheirScore < SCORE_MAX) || OurScore == TheirScore) || DisplayPlay);
